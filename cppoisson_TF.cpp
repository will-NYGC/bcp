...
class cppoisson {
  ...
  public:
    ...
    //cppoisson::cppoisson(Matrix<int>,double,double,double,int,int);
    //cppoisson::~cppoisson(){};
    cppoisson(Matrix<int>,double,double,double,int,int);
    ~cppoisson(){};		
    void        BcmixSmooth();
    Matrix<double> pc;
    void        ratio(Matrix<double> &pc1);
  ...
  private:
    ...
    void    SubFun1(double&, double&, double&, double, double, int, int, int);
    void    BcmixForward();
    void    BcmixBackward();
...
*/
cppoisson::cppoisson(Matrix<int> obs1, double p1,
...
void cppoisson::ratio(Matrix<double> & pc1)
{
  double        total, logij, tmpmax;
  int start,end,n1,t,cp_j;
  Vector<double>        tmpA(K+1);
  Matrix<double>        tmpAlpha(K+1,K+1),tmpIBeta(K+1,K+1),tmpB(K+1,K+1);
  n1 = pc1.num_rows();	
  for (int cp_j=0;cp_j<n1;cp_j++){     
	total=0.0;      tmpmax=-1.0e+300; 
	t = (int)(pc1[cp_j][3]+0.1);	start=(int)(pc1[cp_j][0]+0.1);end=(int)(pc1[cp_j][1]+0.1);
	...
	for (int i=1;i<=min(t,K);i++) {     
    	...
    	for (int i=1;i<=min(t,K);i++) for (int j=1;j<=min(N-t,K);j++) {
      		...
      		tmpIBeta[i][j]=fIbeta[t][i]+bIbeta[t+1][j]-invbeta;
      		logij=LogGamma(tmpAlpha[i][j])-tmpAlpha[i][j]*log(tmpIBeta[i][j]);
      		tmpB[i][j]= log(1-p) + log(fQ[t][i]) + log(bQ[t+1][j]) +logij + log00 -fLog[t][i] - bLog[t+1][j];
      		if (tmpmax<tmpB[i][j])    tmpmax=tmpB[i][j];
    	}

     	for (int i=1; i<=min(t,K); i++) {
		total += tmpA[i] = exp(tmpA[i]-tmpmax);
		if ((start<=fFilter[t][i])&&(fFilter[t][i]<=end))
      			pc1[cp_j][5] += tmpA[i]*falpha[t][i]/fIbeta[t][i];
    	}

    	for (int i=1;i<=min(t,K);i++) for (int j=1;j<=min(N-t,K);j++) {
		total += tmpB[i][j] = exp(tmpB[i][j]-tmpmax);
                if ((start<=fFilter[t][i])&&(fFilter[t][i]<=end)&&(start<=bFilter[t][j])&&(bFilter[t][j]<=end))
      			pc1[cp_j][5] += tmpB[i][j]*tmpAlpha[i][j]/tmpIBeta[i][j];
	}

    	pc1[cp_j][5] /= total;
	pc1[cp_j][6] = pc1[cp_j][5]/pc1[cp_j][4];
	if (pc1[cp_j][6]>1.00000001) cout<<"#####"<<endl<<pc1[cp_j][3]<<"\t"<<pc1[cp_j][6]<<endl;
  }
  WriteMatrix("pc-rep.txt",pc1);
}
...
void cppoisson::BcmixBackward()
{
  ...
  SubFun1(balpha[N][1],bIbeta[N][1],bLog[N][1],alpha,invbeta,obs[N][2],obs[N][0],obs[N][1]);
  for (int t=N-1; t>=N-K+1; t--) {      s=N+1-t;
    SubFun1(tmpPost[s][1],tmpPost[s][2],tmpPost[s][3],alpha,invbeta,obs[t][2],obs[t][0],obs[t][1]);
    numTmp[s] = tmpPost[s][3]-log00;
    for (int i=1; i<s; i++) {
      SubFun1(tmpPost[i][1],tmpPost[i][2],tmpPost[i][3],
        balpha[t+1][i], bIbeta[t+1][i], obs[t][2],obs[t][0],obs[t][1]);
  ...
  for (int t=N-K; t>=1; t--) {          s=N+1-t;
    SubFun1(tmpPost[K+1][1],tmpPost[K+1][2],tmpPost[K+1][3],
      alpha,invbeta, obs[t][2],obs[t][0],obs[t][1]);
  ...
  //cout<<bFilter[5797][1]<<"\t"<<bFilter[8352][1]<<endl;
}
...
void cppoisson::BcmixForward()
{
  ...
  SubFun1(falpha[1][1],fIbeta[1][1],fLog[1][1],alpha,invbeta,obs[1][2],obs[1][0],obs[1][1]);
  for (int t=2; t<=K; t++) {
    SubFun1(tmpPost[t][1],tmpPost[t][2],tmpPost[t][3],alpha,invbeta,obs[t][2],obs[t][0],obs[t][1]);
    numTmp[t]=tmpPost[t][3]-log00;
    for (int i=1; i<t; i++) {
      SubFun1(tmpPost[i][1],tmpPost[i][2],tmpPost[i][3],falpha[t-1][i],
        fIbeta[t-1][i],obs[t][2],obs[t][0],obs[t][1]);
  ...
  //cout<<fFilter[5797][10]<<"\t"<<fFilter[8352][10]<<endl;
...
{
  newalpha = oldalpha + (double)((end-start+1)*count);
  newinvbeta = oldinvbeta + (double)(end-start+1);
