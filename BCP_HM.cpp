...
{	
	...
	while ( (next_option = getopt(argc, argv, "1:2:f:w:p:3:h:")) != -1) {
        ...
        switch (next_option) {
			...
			case 'f':		sscanf (optarg,"%d",&frag_size);        break;
			case 'w':       sscanf (optarg,"%d",&win_size);		break;
			case 'p':       sscanf (optarg,"%lf",&p_value); break;

			case '3':		Seg_Results = optarg;	break;
  ...
  {	
	...
	while(!fin.eof())
	{
		...
		if(chrNum<N)
		{
			...
			else if(chrSign=='-' && b>frag_size) Minus_data[chrNum].push_back(b+1-frag_size);
		}
	}
	fin.close();
	fin.clear();
	...
	while(!fin.eof())
	{
		...
		if(chrNum<N)
		{
			...
			else if(chrSign=='-' && b>frag_size) Minus_input[chrNum].push_back(b+1-frag_size);
		}
	}
	fin.close();


	out1 = fopen(Seg_Results,"w");
	...
	for(int i=N0; i<N; i++)
	{
		...
		num_data_frag = num;// in fact the num_data should be num-1, just for convenient for qsort;
		...
		num_input_frag = num;// in fact the num_input should be num-1, just for convenient for qsort;
		cout<<"number of input fragments:"<<num_input_frag<<endl;
		qsort(input_frag,num_input_frag,sizeof(int),cmp);

		cout<<"finish pre-processing data of chr"<<i<<endl;
		...
		{	
			...
			for(m=1;m<num_win;m++)
				if ((obs[m][2]==0)&&(obs[m][3]==1)&&((obs[m-1][2]!=0)||(obs[m+1][2]!=0))) obs[m][3]=0;
			...
			if (win_size>=200) {t1 =10;t2=5;}
			if (win_size<200 && win_size>=100){t1=7;t2=4;}
			if (win_size<100){t1=5;t2=3;}
			if (t1>=num_win) {t1=5;t2=3;}
			if (t1>=num_win) {cout<<"Too less data for the model."<<endl; exit(1);}
			...
			cout<<"finish calculate posterior means of chr"<<i<<endl;
...
/*
                        sprintf(name4print,"rseg_chr%d",i);
                        out4= fopen(name4print,"w");
                        for (m=1;m<=num_seg;m++)
                                fprintf(out4,"%d %d %d %lf %d %lf %lf\n", (int)rseg[m][0],(int)rseg[m][1],(int)rseg[m][2],rseg[m][3],(int)rseg[m][4],rseg[m][5],rseg[m][6]);
                        fclose(out4);
*/
			...
			if (i==25) sprintf(name4print,"chrM");
			  ...
			  /*fprintf(out1, "%s\t%d\t%d\t%d\t%lf\n",name4print,(int)(rseg[m][0]-1),(int)rseg[m][1],(int)rseg[m][2],rseg[m][3]);*/
			  fprintf(out1, "%s\t%d\t%d\t%lf\n",name4print,(int)(rseg[m][0]),(int)(rseg[m][1]+1),rseg[m][3]);
			delete [] rseg;
			delete [] ss;
			delete [] win_data;

			cout<<"Finish choose significant segments of chr"<<i<<endl;

		}
		else cout<<"chr"<<i<<"is empty."<<endl;
...
void printhelp()
{
	...
	cout<<"\t-f\tThe fragment size to which we extend the reads in pre-processing data step. Default:200bp."<<endl;
	cout<<"\t-w\tThe window size we apply the adjcaent window in pre-processing data step. Default:200bp."<<endl;
    cout<<"\t-p\tThe p_value you want to use for remove false positive based on control data. Range:1e-2---1e-6, default is 1e-3."<<endl;
    cout<<"\t-3\tThe results data set with 5 columns you want to output."<<endl;
	cout<<"\t-h\tThe flag indicates whether you want to output the help manual. 1 means displaying the help instructions while 0 means NOT. Default: 0. If you set the flag as 1, the program would not works except displaying the help manual."<<endl;
}
